...
function MacroEngine:SetupPhysics(root)
    ...
    local AP_Mover = Instance.new("AlignPosition")
    ...
    local AO_Mover = Instance.new("AlignOrientation")
    ...
    self.PhysicsProps.Att = MoverAttachment
    self.PhysicsProps.AP = AP_Mover
    self.PhysicsProps.AO = AO_Mover
    
    table.insert(self.Bin, MoverAttachment)
    table.insert(self.Bin, AP_Mover)
    table.insert(self.Bin, AO_Mover)
end
...
function MacroEngine:GetClosestFrame(currentPos)
    ...
    for i, frame in ipairs(self.Frames) do
        local fPos = Vector3.new(frame.px, frame.py, frame.pz)
...
function MacroEngine:Record()
    ...
    table.insert(self.Bin, RunService.Heartbeat:Connect(function()
        ...
        local currentRot = root.CFrame.LookVector
        local currentToolObj = char:FindFirstChildOfClass("Tool")
        local currentToolName = currentToolObj and currentToolObj.Name or nil
        local isJump = hum.Jump or hum:GetState() == Enum.HumanoidStateType.Jumping
        
        local dist = (currentPos - lastPos).Magnitude
        local angle = math.acos(math.clamp(currentRot:Dot(lastRot), -1, 1))
        ...
        if dist > 0.1 or angle > 0.017 or isJump or self.IsM1Down ~= lastM1 or currentToolName ~= lastTool then
            ...
            table.insert(self.Frames, {
                ...
                lx = currentRot.X, ly = currentRot.Y, lz = currentRot.Z,
...
function MacroEngine:Play()
    ...
    table.insert(self.Bin, RunService.Stepped:Connect(function(_, dt)
        ...
        if pF then
            ...
            local pPos = Vector3.new(pF.px, pF.py, pF.pz)
            local cPos = Vector3.new(cF.px, cF.py, cF.pz)
            local pLook = Vector3.new(pF.lx, pF.ly, pF.lz)
            local cLook = Vector3.new(cF.lx, cF.ly, cF.lz)
            local pMD = Vector3.new(pF.mdx, pF.mdy, pF.mdz)
            local cMD = Vector3.new(cF.mdx, cF.mdy, cF.mdz)

            local lerpedPos = pPos:Lerp(cPos, alpha)
            local lerpedLook = pLook:Lerp(cLook, alpha)
            local lerpedMD = pMD:Lerp(cMD, alpha)

            if self.PhysicsProps.Att then
                if lerpedLook.Magnitude > 0.001 then
                    self.PhysicsProps.Att.CFrame = CFrame.lookAt(lerpedPos, lerpedPos + lerpedLook)
                else
                    self.PhysicsProps.Att.CFrame = CFrame.new(lerpedPos)
                end
            end

            if lerpedMD.Magnitude > 0.05 then
